...
if __name__ == '__main__':
    ...
    #载入自定义词典
    temp_file="test.csv"
    jieba.load_userdict("our_data/data/stock_dict.txt")
    stop_words=pd.read_csv('our_data/data/stopwords.txt', sep='\n', encoding='utf8',header=None)
    stopwords=(stop_words[0]).tolist()
    text=""
    lines=[]
    while 1:
        print ("请输入想要判断极性的标题，退出请输入“break”：")
        line=input()
        if line=="break":
            break
        else:
    ...
    train_dir="./trained_results_1514094680/"
    predict_unseen_data(train_dir,temp_file)